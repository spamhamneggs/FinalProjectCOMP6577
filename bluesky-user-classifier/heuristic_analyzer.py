...
def load_term_database(csv_file: str) -> pd.DataFrame:
    ...
    try:
        ...
        return df[["term", "combined_score"]].sort_values(by="combined_score", ascending=False)
...
def calculate_category_score(text_content: str, terms_df: pd.DataFrame) -> float:
    ...
    """
    if terms_df.empty or not isinstance(text_content, str) or terms_df["combined_score"].sum() == 0:
        return 0.0
    ...
    if not terms_df["term"].apply(isinstance, args=(str,)).all():
        print("Warning: Non-string values found in 'term' column of terms_df during score calculation.")
    ...
    return matched_terms_score_sum / max_potential_score if max_potential_score > 0 else 0.0
...
def determine_classification_labels_category_specific(
    ...
    # Case D: Both Weeb and Furry have signal (neither is "Normie" strength)
        ...
        if w_score >= f_score: # Weeb is dominant or scores are equal
            ...
            if weeb_strength_category == "Strong": primary_label = "Weeb"
            else: primary_label = "Slight Weeb" # Must be "Slight" as it's not "Normie"
            
            # Assign Furry as secondary based on its strength
            if furry_strength_category == "Strong": secondary_label = "Furry"
            elif furry_strength_category == "Slight": secondary_label = "Slight Furry"
            else: secondary_label = "None" # Should not happen if furry_strength_category != "Normie"
        
        else: # Furry is dominant
            # Assign Furry as primary based on its strength
            if furry_strength_category == "Strong": primary_label = "Furry"
            else: primary_label = "Slight Furry" # Must be "Slight"
            
            # Assign Weeb as secondary based on its strength
            if weeb_strength_category == "Strong": secondary_label = "Weeb"
            elif weeb_strength_category == "Slight": secondary_label = "Slight Weeb"
            else: secondary_label = "None" # Should not happen

        # Final check: Prevent self-secondary if base types are the same
        # (e.g. "Weeb" primary and "Slight Weeb" secondary is not desired)
        primary_base_type = "Weeb" if "Weeb" in primary_label else ("Furry" if "Furry" in primary_label else "Normie")
        secondary_base_type = "Weeb" if "Weeb" in secondary_label else ("Furry" if "Furry" in secondary_label else "None")

        if primary_base_type != "Normie" and primary_base_type == secondary_base_type and secondary_base_type != "None":
            secondary_label = "None"
...
def analyze_heuristics(
    ...
    if not force_recalculate_scores and os.path.exists(scores_csv_path):
        ...
        try:
            ...
            if not {'text_snippet', 'weeb_score', 'furry_score'}.issubset(results_df.columns):
                print("Pre-calculated scores file is missing required columns. Recalculating.")
                results_df = None 
        except Exception as e:
    ...
    if results_df is None:
        ...
        try:
            df_text_input = pd.read_csv(data_csv_path, usecols=['text'], on_bad_lines='skip')
        ...
        scores_data = []
        for _, row in tqdm(df_text_input.iterrows(), total=len(df_text_input), desc="Calculating scores"):
            text = row['text']
            if not text.strip():
                continue
    ...
    if not results_df['weeb_score'].empty:
        actual_weeb_normie_cutoff = np.percentile(results_df['weeb_score'], weeb_normie_p)
        actual_weeb_strong_cutoff = np.percentile(results_df['weeb_score'], weeb_strong_p)
        if actual_weeb_strong_cutoff < actual_weeb_normie_cutoff:
            print(f"Warning: Weeb strong percentile ({weeb_strong_p}th) score "
        ...
        print(f"  Weeb Normie Cutoff (at {weeb_normie_p}th percentile of weeb_scores): {actual_weeb_normie_cutoff:.6f}")
        print(f"  Weeb Strong Cutoff (at {weeb_strong_p}th percentile of weeb_scores): {actual_weeb_strong_cutoff:.6f}")
    else:
        print("Warning: No weeb_scores to calculate percentiles from. Using 0.0 for weeb cutoffs.")

    # For Furry scores
    actual_furry_normie_cutoff = 0.0
    actual_furry_strong_cutoff = 0.0
    if not results_df['furry_score'].empty:
        actual_furry_normie_cutoff = np.percentile(results_df['furry_score'], furry_normie_p)
        actual_furry_strong_cutoff = np.percentile(results_df['furry_score'], furry_strong_p)
        if actual_furry_strong_cutoff < actual_furry_normie_cutoff:
            print(f"Warning: Furry strong percentile ({furry_strong_p}th) score "
        ...
        print(f"  Furry Normie Cutoff (at {furry_normie_p}th percentile of furry_scores): {actual_furry_normie_cutoff:.6f}")
        print(f"  Furry Strong Cutoff (at {furry_strong_p}th percentile of furry_scores): {actual_furry_strong_cutoff:.6f}")
    else:
        print("Warning: No furry_scores to calculate percentiles from. Using 0.0 for furry cutoffs.")
    ...
    labels_list = []
    for _, row in tqdm(results_df.iterrows(), total=len(results_df), desc="Applying category-specific labels"):
        primary_label, secondary_label = determine_classification_labels_category_specific(
            row['weeb_score'], 
            row['furry_score'],
    ...
    final_results_df = pd.concat([results_df.reset_index(drop=True), labels_df.reset_index(drop=True)], axis=1)

    # Output Generation
    print(f"\nSaving analysis results to {current_output_dir}...")

    label_distribution = final_results_df["combined_label"].value_counts(normalize=True).mul(100).round(2)
    print("\n--- Heuristic Label Distribution (%) ---")
    print(label_distribution)
    label_distribution.to_csv(os.path.join(current_output_dir, "label_distribution.csv"))
    ...
    plt.suptitle(f"Score Distributions (Percentiles: WN{weeb_normie_p},WS{weeb_strong_p}, FN{furry_normie_p},FS{furry_strong_p})", fontsize=12)
    plt.subplot(1, 2, 1)
    sns.histplot(results_df["weeb_score"], kde=True, bins=30)
    plt.title("Weeb Score Distribution")
    plt.xlabel("Normalized Weeb Score")
    plt.ylabel("Frequency")
    plt.axvline(actual_weeb_normie_cutoff, color='orange', linestyle='--', label=f'Normie Cutoff ({actual_weeb_normie_cutoff:.4f}) @ P{weeb_normie_p}')
    plt.axvline(actual_weeb_strong_cutoff, color='red', linestyle='--', label=f'Strong Cutoff ({actual_weeb_strong_cutoff:.4f}) @ P{weeb_strong_p}')
    ...
    plt.ylabel("Frequency")
    plt.axvline(actual_furry_normie_cutoff, color='orange', linestyle='--', label=f'Normie Cutoff ({actual_furry_normie_cutoff:.4f}) @ P{furry_normie_p}')
    plt.axvline(actual_furry_strong_cutoff, color='red', linestyle='--', label=f'Strong Cutoff ({actual_furry_strong_cutoff:.4f}) @ P{furry_strong_p}')
    plt.legend(fontsize='small')

    plt.tight_layout(rect=[0, 0, 1, 0.95])
    histograms_path = os.path.join(current_output_dir, "score_histograms_with_cutoffs.png")
    plt.savefig(histograms_path)
    print(f"Saved score histograms with cutoffs to {histograms_path}")
    plt.close()
    
    detailed_data_path = os.path.join(current_output_dir, "heuristic_labelled_data_details.csv")
...
if __name__ == "__main__":
    ...
    if args.weeb_strong_percentile < args.weeb_normie_percentile:
        print(f"Error: --weeb-strong-percentile ({args.weeb_strong_percentile}) cannot be less than --weeb-normie-percentile ({args.weeb_normie_percentile}).")
        exit(1)
    if args.furry_strong_percentile < args.furry_normie_percentile:
        print(f"Error: --furry-strong-percentile ({args.furry_strong_percentile}) cannot be less than --furry-normie-percentile ({args.furry_normie_percentile}).")
        exit(1)

    weeb_terms = load_term_database(args.weeb_terms_csv)
    furry_terms = load_term_database(args.furry_terms_csv)

    if (weeb_terms.empty or furry_terms.empty) and not (args.force_recalculate_scores or os.path.exists(os.path.join(args.output_dir, "calculated_scores_for_data.csv"))):
        print("Warning: One or both term databases are empty. If not forcing score recalculation and no cached scores exist, analysis might be based on incomplete term data.")
        if weeb_terms.empty and furry_terms.empty:
             print("Error: Both term databases are empty, and cannot proceed without terms if scores need to be calculated. Exiting.")
             exit(1)
