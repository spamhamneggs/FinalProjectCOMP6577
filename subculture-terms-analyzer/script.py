...
class SubcultureTermAnalyzer:
    def __init__(self, culture_type="weeb"):
        ...
        if culture_type == "weeb":
            ...
            self.seed_terms = {
                "furry", "fursuitfriday", "fursuit", "uwu", 
                "owo", "paws", "yiff", "fursona"
            }
    ...
    def compute_term_specificity(self, df):
        ...
        culture_freqs = np.array(culture_matrix.sum(axis=0)).flatten() / len(culture_corpus)
    ...
    def train_word2vec(self, df):
        ...
        sentences = [
            text.split() for text in tqdm(df["cleaned_text"], desc="Preparing Word2Vec data") 
    ...
    def analyze(self):
        ...
        final_terms.sort_values("combined_score", ascending=False).to_csv(output_file, index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--clear-cache', action='store_true', help='Clear preprocessing cache before running')
