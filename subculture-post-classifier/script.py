...
class SubculturePostClassifier:
    ...
    def _process_batch(self, texts):
        ...
        for i in range(term_counts.shape[0]):
            ...
            weeb_terms = dict(sorted(weeb_terms.items(), key=lambda x: x[1], reverse=True))
            furry_terms = dict(sorted(furry_terms.items(), key=lambda x: x[1], reverse=True))
            
            # Determine categories
            if weeb_score > furry_score:
                if weeb_score > self.threshold:
                    primary_category = PostCategory.WEEB
                    subcategory = PostCategory.FURRY if furry_score > self.threshold * 0.8 else None
                else:
                ...
                if furry_score > self.threshold:
                    ...
                    subcategory = PostCategory.WEEB if weeb_score > self.threshold * 0.8 else None
                else:
                    primary_category = PostCategory.NORMIE
                    subcategory = None
    ...
    def classify_posts(self, df, text_column="text", n_jobs=-1, chunk_size=100000, temp_dir="./cache"):
        ...
        with tqdm(total=chunks, 
                 desc="Processing chunks", 
                 position=0, 
                 bar_format=bar_format,
                 dynamic_ncols=True) as pbar_chunks:
            ...
            for chunk_idx in range(chunks):
                ...
                logger.info(f"Processing chunk {chunk_idx + 1}/{chunks} (rows {start_idx}-{end_idx})")
                ...
                batch_indices = [(i * self.batch_size, min((i + 1) * self.batch_size, len(chunk_df))) 
                ...
                all_results = []
                with tqdm(total=len(chunk_df), 
                         desc=f"Current chunk ({chunk_idx + 1}/{chunks})",
                         position=1, 
                         leave=False,
                         bar_format=bar_format,
                         dynamic_ncols=True) as pbar_batch:
                    ...
                    for batch_idx, batch_result in enumerate(batch_results):
                        ...
                        batch_size = batch_indices[batch_idx][1] - batch_indices[batch_idx][0]
                ...
                logger.info(f"Chunk {chunk_idx + 1}/{chunks} completed and saved to {chunk_file}")
